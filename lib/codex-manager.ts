...
import {
	...
	runReport as runReportCommand,
} from "./codex-manager/forecast-report-commands.js";
import {
	runDoctor as runDoctorCommand,
	runFix as runFixCommand,
	runVerifyFlagged as runVerifyFlaggedCommand,
...
async function runVerifyFlagged(args: string[]): Promise<number> {
	return runVerifyFlaggedCommand(args, repairCommandDeps);
}

async function runFix(args: string[]): Promise<number> {
	return runFixCommand(args, repairCommandDeps);
}

async function runDoctor(args: string[]): Promise<number> {
	return runDoctorCommand(args, repairCommandDeps);
}
...
async function runForecast(args: string[]): Promise<number> {
	return runForecastCommand(args, forecastReportCommandDeps);
}

async function runReport(args: string[]): Promise<number> {
	return runReportCommand(args, forecastReportCommandDeps);
}
...
const forecastReportCommandDeps = {
	stylePromptText,
	styleQuotaSummary,
	formatResultSummary,
	resolveActiveIndex,
