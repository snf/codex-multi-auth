...
import { createAccountManagerReloader, createPersistAccounts, runRuntimeOAuthFlow } from "../lib/runtime/auth-facade.js";
...
describe("createPersistAccounts", () => {
	it("forwards persist dependencies and replaceAll flag", async () => {
		...
		const persistAccounts = createPersistAccounts({ persistAccountPool, withAccountStorageTransaction: vi.fn(), extractAccountId: vi.fn(), extractAccountEmail: vi.fn(), sanitizeEmail: vi.fn(), findMatchingAccountIndex: vi.fn(), MODEL_FAMILIES: ["codex"] });
		const results = [{ refreshToken: "r1" }] as never[];
		await persistAccounts(results, true);
		expect(persistAccountPool).toHaveBeenCalledWith(results, true, expect.objectContaining({ MODEL_FAMILIES: ["codex"] }));
	});
});
...
describe("createAccountManagerReloader", () => {
	it("forwards auth fallback and current reload state", async () => {
		...
		const reloader = createAccountManagerReloader({ reloadRuntimeAccountManager, getReloadInFlight: () => null, loadFromDisk: vi.fn(async () => "manager"), setCachedAccountManager: vi.fn(), setAccountManagerPromise: vi.fn(), setReloadInFlight: vi.fn() });
		await expect(reloader({ type: "oauth", access: "a", refresh: "r", expires: 1 })).resolves.toBe("manager");
		expect(reloadRuntimeAccountManager).toHaveBeenCalledWith(expect.objectContaining({ authFallback: expect.objectContaining({ refresh: "r" }) }));
	});
});
