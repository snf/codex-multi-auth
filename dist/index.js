...
import { classifyRefreshFailureForReauth, clearAccountReauthRequired, markAccountReauthRequired, } from "./lib/account-reauth.js";
...
import { getAutoResume, getCodexMode, getEmptyResponseMaxRetries, getEmptyResponseRetryDelayMs, getFallbackToGpt52OnUnsupportedGpt53, getFastSession, getFastSessionMaxInputItems, getFastSessionStrategy, getFetchTimeoutMs, getLiveAccountSync, getLiveAccountSyncDebounceMs, getLiveAccountSyncPollMs, getNetworkErrorCooldownMs, getPerProjectAccounts, getPidOffsetEnabled, getPreemptiveQuotaEnabled, getPreemptiveQuotaMaxDeferralMs, getPreemptiveQuotaRemainingPercent5h, getPreemptiveQuotaRemainingPercent7d, getProactiveRefreshBufferMs, getProactiveRefreshGuardian, getProactiveRefreshIntervalMs, getRateLimitToastDebounceMs, getRetryAllAccountsMaxRetries, getRetryAllAccountsMaxWaitMs, getRetryAllAccountsRateLimited, getBackgroundResponses, getResponseContinuation, getServerErrorCooldownMs, getSessionAffinity, getSessionAffinityMaxEntries, getSessionAffinityTtlMs, getSessionRecovery, getStorageBackupEnabled, getStreamStallTimeoutMs, getToastDurationMs, getTokenRefreshSkewMs, getUnsupportedCodexFallbackChain, getUnsupportedCodexPolicy, loadPluginConfig, } from "./lib/config.js";
import { ACCOUNT_LIMITS, AUTH_LABELS, CODEX_BASE_URL, DUMMY_API_KEY, LOG_STAGES, PLUGIN_NAME, PROVIDER_ID, } from "./lib/constants.js";
...
import { clearCorrelationId, initLogger, logDebug, logError, logInfo, logRequest, logWarn, setCorrelationId, } from "./lib/logger.js";
...
import { detectErrorType, getRecoveryToastContent, isRecoverableError, } from "./lib/recovery.js";
import { RefreshGuardian } from "./lib/refresh-guardian.js";
import { queuedRefresh } from "./lib/refresh-queue.js";
import { parseEnvInt, parseFailoverMode, } from "./lib/request/failover-config.js";
import { evaluateFailurePolicy, } from "./lib/request/failure-policy.js";
import { applyProxyCompatibleInit, createCodexHeaders, extractRequestUrl, getUnsupportedCodexModelInfo, handleErrorResponse, handleSuccessResponse, isWorkspaceDisabledError, refreshAndUpdateToken, resolveUnsupportedCodexFallbackModel, rewriteUrlForCodex, shouldRefreshToken, transformRequestForCodex, } from "./lib/request/fetch-helpers.js";
...
import { normalizeRequestInit, parseRequestBodyFromInit, } from "./lib/request/request-init.js";
...
import { parseRetryAfterHintMs, sanitizeResponseHeadersForLog, } from "./lib/request/response-metadata.js";
import { withStreamingFailover } from "./lib/request/stream-failover.js";
import { createAbortableSleep, sleepWithCountdown, } from "./lib/request/wait-utils.js";
import { addJitter } from "./lib/rotation.js";
import { clampActiveIndices, isFlaggableFailure, } from "./lib/runtime/account-check-helpers.js";
...
import { invalidateRuntimeAccountManagerCache, reloadRuntimeAccountManager, } from "./lib/runtime/account-manager-cache.js";
...
import { formatRateLimitEntry, getRateLimitResetTimeForFamily, resolveActiveIndex, } from "./lib/runtime/account-status.js";
...
import { ensureLiveAccountSyncState, ensureRefreshGuardianState, ensureSessionAffinityState, } from "./lib/runtime/runtime-services.js";
...
import { applyUiRuntimeFromConfig, getStatusMarker, } from "./lib/runtime/ui-runtime.js";
...
import { clearAccounts, clearFlaggedAccounts, exportAccounts, findMatchingAccountIndex, formatStorageErrorHint, getStoragePath, importAccounts, loadAccounts, loadFlaggedAccounts, StorageError, saveAccounts, saveFlaggedAccounts, setStorageBackupEnabled, setStoragePath, withAccountStorageTransaction, } from "./lib/storage.js";
...
import { formatUiBadge, formatUiHeader, formatUiItem, formatUiKeyValue, formatUiSection, paintUiText, } from "./lib/ui/format.js";
...
export const OpenAIOAuthPlugin = async ({ client }) => {
    ...
    const resolveTokenSuccessAccount = (tokens) => resolveAccountSelection(tokens, {
    ...
    const makeReloadAccountManagerDeps = (authFallback) => ({
    ...
    const reloadAccountManagerFromDisk = async (authFallback) => reloadRuntimeAccountManager(makeReloadAccountManagerDeps(authFallback));
    const applyAccountStorageScope = (pluginConfig) => applyAccountStorageScopeEntry({
        pluginConfig,
    ...
    const ensureLiveAccountSync = async (pluginConfig, authFallback) => {
        const next = await ensureLiveAccountSyncEntry({
            ...
            getLiveAccountSync,
            getStoragePath,
            createSync: (oauthFallback) => new LiveAccountSync(async () => {
                await reloadRuntimeAccountManager(makeReloadAccountManagerDeps(oauthFallback));
            }, {
    ...
    const ensureRefreshGuardian = (pluginConfig) => {
        const next = ensureRefreshGuardianState({
            ...
            createGuardian: ({ intervalMs, bufferMs }) => new RefreshGuardian(() => cachedAccountManager, {
                intervalMs,
                bufferMs,
            }),
            registerCleanup,
    ...
    const ensureSessionAffinity = (pluginConfig) => {
        const next = ensureSessionAffinityState({
            ...
            createStore: ({ ttlMs, maxEntries }) => new SessionAffinityStore({ ttlMs, maxEntries }),
        });
        sessionAffinityStore = next.sessionAffinityStore;
        sessionAffinityConfigKey = next.sessionAffinityConfigKey;
    };
    const applyPreemptiveQuotaSettings = (pluginConfig) => applyPreemptiveQuotaSettingsFromConfig(pluginConfig, {
    ...
    const eventHandler = async (input) => handleRuntimeEvent({
        input,
        ...
        showToast: (message, variant) => showRuntimeToast(client, message, variant),
        logDebug,
        pluginName: PLUGIN_NAME,
    });
    ...
    return {
        ...
        auth: {
            ...
            async loader(getAuth, provider) {
                ...
                applyLoaderRuntimeSetup({
                    pluginConfig,
                    applyUiRuntimeFromConfig: (config) => applyUiRuntimeFromConfig(config, setUiRuntimeOptions),
                ...
                try {
                    ...
                    if (!accountManagerPromise) {
                        await reloadRuntimeAccountManager(makeReloadAccountManagerDeps(auth));
                    }
                    const managerPromise = accountManagerPromise ??
                        reloadRuntimeAccountManager(makeReloadAccountManagerDeps(auth));
                    ...
                    const needsPersist = refreshToken && !accountManager.hasRefreshToken(refreshToken);
                    if (needsPersist) {
                    ...
                    const maxSameAccountRetries = failoverMode === "conservative"
                        ? 2
                    ...
                    return {
                        ...
                        async fetch(input, init) {
                            try {
                                ...
                                const originalBody = await parseRequestBodyFromInit(baseInit?.body, logWarn);
                                const isStreaming = originalBody.stream === true;
                                const parsedBody = Object.keys(originalBody).length > 0
                                    ? { ...originalBody }
                                    : undefined;
                                ...
                                let pendingFastSessionInputTrim = transformation?.deferredFastSessionInputTrim;
                                ...
                                const responseContinuationEnabled = getResponseContinuation(pluginConfig);
                                ...
                                const effectivePromptCacheKey = (sessionAffinityKey ?? promptCacheKey ?? "")
                                    .toString()
                                    .trim() || undefined;
                                const shouldUseResponseContinuation = Boolean(transformedBody) &&
                                    responseContinuationEnabled &&
                                    !transformedBody?.previous_response_id;
                                if (shouldUseResponseContinuation && transformedBody) {
                                    const lastResponseId = sessionAffinityStore?.getLastResponseId(sessionAffinityKey);
                                    if (lastResponseId) {
                                        transformedBody = {
                                            ...transformedBody,
                                ...
                                const requestCorrelationId = setCorrelationId(threadIdCandidate
                                ...
                                while (true) {
                                    ...
                                    const accountSnapshotList = typeof accountSnapshotSource.getAccountsSnapshot ===
                                        "function"
                                    ...
                                    for (const candidate of accountSnapshotList) {
                                        ...
                                        capabilityBoostByAccount[candidate.index] =
                                            capabilityPolicyStore.getBoost(accountKey, model ?? modelFamily);
                                    }
                                    accountAttemptLoop: while (attempted.size < Math.max(1, accountCount)) {
                                        ...
                                        try {
                                            ...
                                            if (authFailurePolicy.removeAccount) {
                                                ...
                                                accountManager.saveToDiskDebounced();
                                                await showRuntimeToast(client, `Removed ${accountLabel} after ${failures} consecutive auth failures. Run 'codex login' to re-add.`, "error", { duration: toastDurationMs * 2 });
                                                continue;
                                            }
                                        ...
                                        if (accountCount > 1 &&
                                            ...
                                            const accountLabel = formatAccountLabel(account, account.index);
                                            await showRuntimeToast(client, `Using ${accountLabel} (${account.index + 1}/${accountCount})`, "info");
                                        ...
                                        if (transformedBody && pendingFastSessionInputTrim) {
                                            ...
                                            const compactionResult = await applyResponseCompaction({
                                                ...
                                                fetchImpl: async (requestUrl, requestInit) => {
                                                    const normalizedCompactionUrl = typeof requestUrl === "string"
                                                        ? requestUrl
                                                        : String(requestUrl);
                                                    return fetch(normalizedCompactionUrl, applyProxyCompatibleInit(normalizedCompactionUrl, requestInit));
                                                },
                                                signal: abortSignal,
                                        ...
                                        while (true) {
                                            ...
                                            const onUserAbort = abortSignal
                                                ? () => fetchController.abort(abortSignal.reason ??
                                            ...
                                            try {
                                                ...
                                                const isTimeoutAbort = requestTimedOut ||
                                            ...
                                            if (!response.ok) {
                                                ...
                                                const blockedModel = unsupportedModelInfo.unsupportedModel ??
                                                    model ??
                                                    "requested model";
                                                ...
                                                const allowUnsupportedFallback = fallbackOnUnsupportedCodexModel ||
                                                ...
                                                if (fallbackModel) {
                                                    ...
                                                    await showRuntimeToast(client, `Model ${previousModel} is not available for this account. Retrying with ${model}.`, "warning", { duration: toastDurationMs });
                                                ...
                                                if (unsupportedModelInfo.isUnsupported &&
                                                    ...
                                                    await showRuntimeToast(client, `Model ${blockedModel} is not available for this account. Strict policy blocked automatic fallback.`, "warning", { duration: toastDurationMs });
                                                }
                                                if (unsupportedModelInfo.isUnsupported &&
                                                    allowUnsupportedFallback &&
                                                    !hasRemainingAccounts &&
                                                ...
                                                const workspaceErrorCode = errorBody?.error?.code ?? "";
                                                const workspaceErrorMessage = errorBody?.error?.message ?? "";
                                                const isDisabledWorkspaceError = isWorkspaceDisabledError(errorResponse.status, workspaceErrorCode, workspaceErrorMessage);
                                                ...
                                                if (isDisabledWorkspaceError) {
                                                    ...
                                                    if (!account.workspaces ||
                                                        ...
                                                        logWarn(`Workspace disabled/expired for account ${account.index + 1} without tracked workspaces. Leaving account enabled.`, { errorCode: workspaceErrorCode });
                                                        ...
                                                        const workspaceName = currentWorkspace?.name ??
                                                            currentWorkspace?.id ??
                                                            "unknown";
                                                        logWarn(`Workspace disabled/expired for account ${account.index + 1} - workspace: ${workspaceName}. Rotating to next workspace.`, { errorCode: workspaceErrorCode });
                                                        ...
                                                        if (nextWorkspace) {
                                                            ...
                                                            await showRuntimeToast(client, `Workspace ${workspaceName} disabled. Switched to ${newWorkspaceName}.`, "warning", { duration: toastDurationMs });
                                                        ...
                                                        accountManager.saveToDiskDebounced();
                                                        await showRuntimeToast(client, `All workspaces disabled for account ${account.index + 1}. Switching to another account.`, "warning", { duration: toastDurationMs });
                                                ...
                                                if (recoveryHook &&
                                                    ...
                                                    await showRuntimeToast(client, `${toastContent.title}: ${toastContent.message}`, "warning", { duration: toastDurationMs });
                                                ...
                                                if (response.status >= 500 && response.status < 600) {
                                                    ...
                                                    const policy = evaluateFailurePolicy({
                                                        kind: "server",
                                                        failoverMode,
                                                        serverRetryAfterMs: serverRetryAfterMs ?? undefined,
                                                ...
                                                if (rateLimit) {
                                                    ...
                                                    if (delayMs <= RATE_LIMIT_SHORT_RETRY_THRESHOLD_MS) {
                                                        if (accountManager.shouldShowAccountToast(account.index, rateLimitToastDebounceMs)) {
                                                            await showRuntimeToast(client, `Rate limited. Retrying in ${waitLabel} (attempt ${attempt})...`, "warning", { duration: toastDurationMs });
                                                            accountManager.markToastShown(account.index);
                                                        }
                                                    ...
                                                    if (accountManager.getAccountCount() > 1 &&
                                                        accountManager.shouldShowAccountToast(account.index, rateLimitToastDebounceMs)) {
                                                        await showRuntimeToast(client, `Rate limited. Switching accounts (retry in ${waitLabel}).`, "warning", { duration: toastDurationMs });
                                                        accountManager.markToastShown(account.index);
                                                    }
                                                    break;
                                                }
                                                ...
                                                responseForSuccess = withStreamingFailover(response, async (failoverAttempt, emittedBytes) => {
                                                    ...
                                                    for (const candidateIndex of streamFallbackCandidateOrder) {
                                                        ...
                                                        const fallbackEntitlementAccountKey = resolveEntitlementAccountKey({
                                                            accountId: fallbackStoredAccountId ??
                                                                fallbackAccountId,
                                                            email: fallbackResolvedEmail,
                                                            refreshToken: fallbackAccount.refreshToken,
                                                            index: fallbackAccount.index,
                                                        });
                                                        const fallbackEntitlementBlock = entitlementCache.isBlocked(fallbackEntitlementAccountKey, model ?? modelFamily);
                                                        if (fallbackEntitlementBlock.blocked) {
                                                            runtimeMetrics.accountRotations++;
                                                            runtimeMetrics.lastError = `Entitlement cached block for account ${fallbackAccount.index + 1}`;
                                                            logWarn(`Skipping account ${fallbackAccount.index + 1} due to cached entitlement block (${formatWaitTime(fallbackEntitlementBlock.waitMs)} remaining).`);
                                                            continue;
                                                        }
                                                        ...
                                                        const onFallbackAbort = abortSignal
                                                            ? () => fallbackController.abort(abortSignal.reason ??
                                                                new Error("Aborted by user"))
                                                            : null;
                                                        ...
                                                        try {
                                                            ...
                                                            if (fallbackSnapshot) {
                                                                preemptiveQuotaScheduler.update(`${fallbackEntitlementAccountKey}:${model ?? modelFamily}`, fallbackSnapshot);
                                                            }
                                                            if (!fallbackResponse.ok) {
                                                                ...
                                                                if (fallbackResponse.status === 429) {
                                                                    const retryAfterMs = parseRetryAfterHintMs(fallbackResponse.headers) ?? 60_000;
                                                                ...
                                                                capabilityPolicyStore.recordFailure(fallbackEntitlementAccountKey, capabilityModelKey);
                                                                continue;
                                                            }
                                            ...
                                            const successResponse = await handleSuccessResponse(responseForSuccess, isStreaming, {
                                                onResponseId: (responseId) => {
                                                    ...
                                                    sessionAffinityStore?.updateLastResponseId(sessionAffinityKey, responseId);
                                                ...
                                                try {
                                                    ...
                                                    if (isEmptyResponse(parsedBody)) {
                                                        if (emptyResponseRetries < emptyResponseMaxRetries) {
                                                            ...
                                                            await showRuntimeToast(client, `Empty response. Retrying (${emptyResponseRetries}/${emptyResponseMaxRetries})...`, "warning", { duration: toastDurationMs });
                                                            accountManager.refundToken(account, modelFamily, model);
                                                            accountManager.recordFailure(account, modelFamily, model);
                                                            capabilityPolicyStore.recordFailure(entitlementAccountKey, capabilityModelKey);
                                                            ...
                                                            if (emptyPolicy.retrySameAccount &&
                                                                ...
                                                                sameAccountRetryCount += 1;
                                                                runtimeMetrics.sameAccountRetries += 1;
                                                                const retryDelayMs = Math.max(0, Math.floor(emptyPolicy.retryDelayMs ??
                                            ...
                                            if (lastCodexCliActiveSyncIndex !==
                                    ...
                                    if (retryAllAccountsRateLimited &&
                                        ...
                                        await sleepWithCountdown({
                                            ...
                                            sleep,
                                            showToast: (message, variant, options) => showRuntimeToast(client, message, variant, options),
                                            formatWaitTime,
                                            toastDurationMs,
                                            abortSignal,
                                        });
            ...
            methods: [
                {
                    ...
                    authorize: async (inputs) => {
                        ...
                        const accounts = [];
                        const noBrowser = inputs?.manual === "true" ||
                        ...
                        if (!explicitLoginMode) {
                            while (true) {
                                ...
                                const existingAccounts = buildLoginMenuAccounts(workingStorage.accounts, {
                                    now,
                                    activeIndex,
                                    formatRateLimitEntry: (account, currentNow) => formatRateLimitEntry(account, currentNow, formatWaitTime),
                                });
                                ...
                                const accountCheckDeps = {
                                    hydrateEmails: (storage) => hydrateRuntimeEmails(storage, {
                                        queuedRefresh,
                                        extractAccountId,
                                        sanitizeEmail,
                                        extractAccountEmail,
                                    ...
                                    MODEL_FAMILIES,
                                    saveAccounts,
                                    invalidateAccountManagerCache: () => invalidateRuntimeAccountManagerCache(accountManagerCacheInvalidationDeps),
                                    saveFlaggedAccounts,
                        ...
                        const explicitCountProvided = typeof inputs?.accountCount === "string" &&
                        ...
                        while (accounts.length < targetCount) {
                            ...
                            const result = await runRuntimeOAuthFlow(forceNewLogin, {
                                runBrowserOAuthFlow: (input) => runBrowserOAuthFlow({
                                    ...input,
                            ...
                            if (result.type === "success") {
                                ...
                                const accountId = resolved.accountIdOverride ??
                                    extractAccountId(resolved.access);
                                const label = resolved.accountLabel ??
                                    email ??
                                    accountId ??
                                    "Unknown account";
                                logInfo(`Authenticated as: ${label}`);
                                const isDuplicate = findMatchingAccountIndex(accounts.map((account) => ({
                                    accountId: account.accountIdOverride ??
                                        extractAccountId(account.access),
                                    email: sanitizeEmail(extractAccountEmail(account.access, account.idToken)),
                            ...
                            await showRuntimeToast(client, `Account ${accounts.length} authenticated`, "success");
                            try {
        ...
        tool: {
            ...
            "codex-list": tool({
                ...
                args: {},
                async execute() {
                    const ui = applyUiRuntimeFromConfig(loadPluginConfig(), setUiRuntimeOptions);
                    ...
                    if (ui.v2Enabled) {
                        ...
                        storage.accounts.forEach((account, index) => {
                            ...
                            const rateLimit = formatRateLimitEntry(account, now, formatWaitTime);
                            if (rateLimit)
            ...
            "codex-switch": tool({
                ...
                args: {
                    index: tool.schema
                        .number()
                        .describe("Account number to switch to (1-based, e.g., 1 for first account)"),
                },
                async execute({ index }) {
                    const ui = applyUiRuntimeFromConfig(loadPluginConfig(), setUiRuntimeOptions);
                    const storage = await loadAccounts();
                    ...
                    if (cachedAccountManager) {
                        await reloadRuntimeAccountManager(makeReloadAccountManagerDeps());
                    }
                ...
                async execute() {
                    ...
                    if (ui.v2Enabled) {
                        ...
                        storage.accounts.forEach((account, index) => {
                            ...
                            if (account.enabled === false)
                                badges.push(formatUiBadge(ui, "disabled", "danger"));
                            const rateLimit = formatRateLimitEntry(account, now, formatWaitTime) ?? "none";
                        ...
                        for (const family of MODEL_FAMILIES) {
                            ...
                            const familyIndexLabel = typeof idx === "number" && Number.isFinite(idx)
                                ? String(idx + 1)
                                : "-";
                ...
                ? {
                    "codex-remove": tool({
                        ...
                        args: {
                            index: tool.schema
                                .number()
                                .describe("Account number to remove (1-based, e.g., 1 for first account)"),
                        },
                        async execute({ index }) {
                            const ui = applyUiRuntimeFromConfig(loadPluginConfig(), setUiRuntimeOptions);
                            const storage = await loadAccounts();
                    ...
                    "codex-export": tool({
                        ...
                        args: {
                            path: tool.schema
                                .string()
                                .describe("File path to export to (e.g., ~/codex-backup.json)"),
                            force: tool.schema
                                .boolean()
                                .optional()
