...
const BACKUP_WINDOWS_RESERVED_NAME_REGEX = /^(con|prn|aux|nul|com[1-9]|lpt[1-9])$/i;
const BACKUP_INVALID_SUFFIXES = [".tmp", ".wal"];
const BACKUP_PROHIBITED_SUBSTRINGS = [".rotate."];
const inFlightNamedBackupExports = new Set();
/**
 ...
 *         within `baseDir`.
 */
function assertWithinDirectory(baseDir, targetPath) {
    ...
    if (baseStat) {
        ...
        const comparableCanonicalBase = process.platform === "win32"
            ? canonicalBase.toLowerCase()
            : canonicalBase;
        const comparableResolvedBase = process.platform === "win32" ? resolvedBase.toLowerCase() : resolvedBase;
 ...
 */
export function normalizeNamedBackupFileName(name) {
    ...
    if (BACKUP_WINDOWS_RESERVED_NAME_REGEX.test(baseName)) {
        throw new Error("Backup filename may not use a reserved Windows device name");
    }
    if (!BACKUP_SAFE_NAME_REGEX.test(baseName)) {
        throw new Error("Backup filename may only contain letters, numbers, hyphens, and underscores; dots (.) are not allowed");
    }
 ...
 */
export function getNamedBackupRoot(storagePath) {
    const resolvedStoragePath = resolvePath(storagePath);
    return resolvePath(join(dirname(resolvedStoragePath), BACKUP_EXPORT_DIR_NAME));
}
/**
 ...
 */
export function resolveNamedBackupPath(name, storagePath) {
 ...
 */
export async function exportNamedBackupFile(name, dependencies, options) {
    ...
    try {
        await dependencies.exportAccounts(destination, options?.force === true, (resolvedPath) => {
...
//# sourceMappingURL=named-backup-export.js.map