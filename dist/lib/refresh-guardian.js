...
export class RefreshGuardian {
    ...
    constructor(getAccountManager, options = {}) {
    ...
    classifyFailureReason(tokenResult) {
        ...
        if (tokenResult.reason === "network_error")
            return "network-error";
        if (tokenResult.reason === "missing_refresh")
            return "auth-failure";
        if (tokenResult.reason === "invalid_response")
            return "auth-failure";
        if (tokenResult.reason === "http_error" &&
    ...
    async tick() {
        ...
        try {
            ...
            for (const [accountIndex, result] of refreshResults.entries()) {
                ...
                const resolvedIndex = liveAccounts.findIndex((candidate) => candidate.refreshToken === sourceAccount.refreshToken);
