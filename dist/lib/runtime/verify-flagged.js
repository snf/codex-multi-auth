...
export async function verifyRuntimeFlaggedAccounts(deps) {
    ...
    for (let i = 0; i < flaggedStorage.accounts.length; i += 1) {
        ...
        try {
            ...
            if (cached &&
                ...
                deps.showLine(`[${i + 1}/${flaggedStorage.accounts.length}] ${label}: RESTORED (Codex CLI cache)`);
                continue;
            }
            const refreshResult = await deps.queuedRefresh(flagged.refreshToken);
            if (refreshResult.type !== "success") {
                deps.showLine(`[${i + 1}/${flaggedStorage.accounts.length}] ${label}: STILL FLAGGED (${refreshResult.message ?? refreshResult.reason ?? "refresh failed"})`);
            ...
            state.restored.push(resolved);
            deps.showLine(`[${i + 1}/${flaggedStorage.accounts.length}] ${label}: RESTORED`);
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            deps.logError?.(`Failed to verify flagged account ${label}: ${message}`);
            deps.showLine(`[${i + 1}/${flaggedStorage.accounts.length}] ${label}: ERROR (${message.slice(0, 120)})`);
    ...
    deps.showLine("");
    deps.showLine(`Results: ${state.restored.length} restored, ${state.remaining.length} still flagged`);
    deps.showLine("");
}
