export function parseFiniteNumberHeader(headers, name) {
...
export function parseFiniteIntHeader(headers, name) {
    const raw = headers.get(name);
    if (!raw)
        return undefined;
...
export function parseResetAtMs(headers, prefix) {
...
export function hasCodexQuotaHeaders(headers) {
    const keys = [
...
export function parseCodexQuotaSnapshot(headers, status) {
...
export function formatQuotaWindowLabel(windowMinutes) {
...
export function formatResetAt(resetAtMs) {
...
export function formatCodexQuotaLine(snapshot) {
    ...
    if (typeof snapshot.activeLimit === "number" &&
...
//# sourceMappingURL=quota-headers.js.map