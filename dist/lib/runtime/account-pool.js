export async function persistAccountPoolResults(params) {
    ...
    await params.withAccountStorageTransaction(async (loadedStorage, persist) => {
        ...
        for (const result of results) {
            const accountId = result.accountIdOverride ?? params.extractAccountId(result.access);
            ...
            const accountEmail = params.sanitizeEmail(params.extractAccountEmail(result.access, result.idToken));
            const existingIndex = params.findMatchingAccountIndex(accounts, {
                accountId,
                email: accountEmail,
            ...
            if (existingIndex === undefined) {
                const initialWorkspaceIndex = result.workspaces && result.workspaces.length > 0
                    ? (() => {
                        if (accountId) {
                            const matchingWorkspaceIndex = result.workspaces.findIndex((workspace) => workspace.id === accountId);
                            if (matchingWorkspaceIndex >= 0) {
                                return matchingWorkspaceIndex;
                            }
                        }
                        const firstEnabledWorkspaceIndex = result.workspaces.findIndex((workspace) => workspace.enabled !== false);
            ...
            const mergedWorkspaces = result.workspaces
                ...
                : existing.workspaces;
            const currentWorkspaceId = existing.workspaces?.[typeof existing.currentWorkspaceIndex === "number"
                ? existing.currentWorkspaceIndex
                : 0]?.id;
            const nextCurrentWorkspaceIndex = mergedWorkspaces && mergedWorkspaces.length > 0
                ? (() => {
                    if (currentWorkspaceId) {
                        const matchingWorkspaceIndex = mergedWorkspaces.findIndex((workspace) => workspace.id === currentWorkspaceId);
                        if (matchingWorkspaceIndex >= 0) {
                            return matchingWorkspaceIndex;
                        }
                    }
                    const defaultWorkspaceIndex = mergedWorkspaces.findIndex((workspace) => workspace.isDefault === true);
                    if (defaultWorkspaceIndex >= 0) {
                        return defaultWorkspaceIndex;
                    }
                    const firstEnabledWorkspaceIndex = mergedWorkspaces.findIndex((workspace) => workspace.enabled !== false);
                    return firstEnabledWorkspaceIndex >= 0
                        ? firstEnabledWorkspaceIndex
                        : 0;
                })()
