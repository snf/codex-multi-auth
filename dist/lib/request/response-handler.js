...
function createParsedResponseState() {
    return {
        finalResponse: null,
        lastPhase: null,
        outputItems: new Map(),
        outputText: new Map(),
        outputTextPhases: new Map(),
        phaseTextSegments: new Map(),
        phaseSegmentOrder: new Map(),
        phaseText: new Map(),
        reasoningSummaryText: new Map(),
...
function getDeltaField(record, key) {
    const value = record[key];
    return typeof value === "string" && value.length > 0 ? value : null;
}
function isValidSynthesizedIndex(index) {
    return (index !== null &&
        Number.isInteger(index) &&
        index >= 0 &&
        index <= MAX_SYNTHESIZED_EVENT_INDEX);
}
function cloneContentArray(content) {
...
function makeOutputTextKey(outputIndex, contentIndex) {
    if (!isValidSynthesizedIndex(outputIndex) ||
        !isValidSynthesizedIndex(contentIndex)) {
        return null;
    }
    return `${outputIndex}:${contentIndex}`;
}
function makePhaseTextSegmentKey(phase, outputTextKey) {
    return `${phase}\u0000${outputTextKey}`;
}
function makeSummaryKey(outputIndex, summaryIndex) {
    if (!isValidSynthesizedIndex(outputIndex) ||
...
function capturePhase(state, phase) {
    if (typeof phase !== "string" || phase.trim().length === 0)
        return;
    state.lastPhase = phase.trim();
}
function rememberPhaseSegmentOrder(state, phase, segmentKey) {
...
function removePhaseSegmentOrder(state, phase, segmentKey) {
    const existingOrder = state.phaseSegmentOrder.get(phase);
    if (!existingOrder)
        return;
...
function rebuildPhaseText(state, phase) {
...
function setPhaseTextSegment(state, phase, outputTextKey, text) {
    const normalizedPhase = typeof phase === "string" && phase.trim().length > 0
        ? phase.trim()
...
function appendPhaseTextSegment(state, phase, outputTextKey, delta) {
...
function upsertOutputItem(state, outputIndex, item) {
    if (!isValidSynthesizedIndex(outputIndex) || !isRecord(item))
        return;
...
function setOutputTextValue(state, outputIndex, contentIndex, text, phase = undefined) {
...
function appendOutputTextValue(state, outputIndex, contentIndex, delta, phase = undefined) {
    if (!delta)
        return;
    const key = makeOutputTextKey(outputIndex, contentIndex);
    if (!key)
...
function setReasoningSummaryValue(state, outputIndex, summaryIndex, text) {
...
function appendReasoningSummaryValue(state, outputIndex, summaryIndex, delta) {
    if (!delta)
        return;
    const key = makeSummaryKey(outputIndex, summaryIndex);
    if (!key)
...
function ensureOutputItemAtIndex(output, index) {
    if (!isValidSynthesizedIndex(index))
        return null;
    ...
    return isRecord(output[index]) ? output[index] : null;
}
function ensureContentPartAtIndex(item, index) {
    if (!isValidSynthesizedIndex(index))
        return null;
    ...
    return isRecord(content[index]) ? content[index] : null;
}
function applyAccumulatedOutputText(response, state) {
    if (state.outputText.size === 0)
        return;
...
function mergeOutputItemsIntoResponse(response, state) {
    if (state.outputItems.size === 0)
        return;
    const output = Array.isArray(response.output) ? [...response.output] : [];
...
function collectMessageOutputText(output) {
    return output
        .filter(isRecord)
        .map((item) => {
...
function collectReasoningSummaryText(output) {
    return output
        .filter(isRecord)
        .map((item) => {
...
function applyReasoningSummaries(response, state) {
    if (state.reasoningSummaryText.size === 0)
        return;
    const output = Array.isArray(response.output) ? [...response.output] : [];
...
function finalizeParsedResponse(state) {
    ...
    if (state.phaseText.size > 0) {
        ...
        for (const [phase, text] of state.phaseText.entries()) {
            ...
            if (phase === "commentary")
                response.commentary_text = text;
            if (phase === "final_answer")
                response.final_answer_text = text;
        }
...
function extractResponseId(response) {
    if (!response || typeof response !== "object")
        return null;
    const candidate = response.id;
    return typeof candidate === "string" && candidate.trim().length > 0
        ? candidate.trim()
        : null;
}
function notifyResponseId(state, onResponseId, response) {
...
function logStreamDiagnostics(finalResponse) {
...
function maybeCaptureResponseEvent(state, data, onResponseId) {
    if (data.type === "error") {
    ...
    if (data.type === "response.output_text.delta") {
        appendOutputTextValue(state, outputIndex, getNumberField(eventRecord, "content_index"), getDeltaField(eventRecord, "delta"), eventRecord.phase);
        return;
    }
    if (data.type === "response.output_text.done") {
        setOutputTextValue(state, outputIndex, getNumberField(eventRecord, "content_index"), getStringField(eventRecord, "text"), eventRecord.phase);
        return;
    }
    if (data.type === "response.content_part.added" || data.type === "response.content_part.done") {
        ...
        setOutputTextValue(state, outputIndex, getNumberField(eventRecord, "content_index"), getPartText(part), part.phase);
        return;
    }
    if (data.type === "response.reasoning_summary_text.delta") {
        appendReasoningSummaryValue(state, outputIndex, getNumberField(eventRecord, "summary_index"), getDeltaField(eventRecord, "delta"));
        return;
    }
    if (data.type === "response.reasoning_summary_text.done") {
        setReasoningSummaryValue(state, outputIndex, getNumberField(eventRecord, "summary_index"), getStringField(eventRecord, "text"));
        return;
    }
    if (data.type === "response.reasoning_summary_part.added" ||
        data.type === "response.reasoning_summary_part.done") {
        setReasoningSummaryValue(state, outputIndex, getNumberField(eventRecord, "summary_index"), getPartText(eventRecord.part));
        return;
    }
 ...
 */
function parseSseStream(sseText, onResponseId) {
 ...
 */
export async function convertSseToJson(response, headers, options) {
    if (!response.body) {
    ...
    let fullText = '';
    const streamStallTimeoutMs = Math.max(1_000, Math.floor(options?.streamStallTimeoutMs ?? DEFAULT_STREAM_STALL_TIMEOUT_MS));
    try {
...
function createResponseIdCapturingStream(body, onResponseId) {
    const decoder = new TextDecoder();
    let bufferedText = "";
    const state = createParsedResponseState();
        ...
        return await Promise.race([
            ...
            new Promise((_, reject) => {
                timeoutId = setTimeout(() => {
                    reject(new Error(`SSE stream stalled for ${timeoutMs}ms while waiting for a terminal response event`));
                }, timeoutMs);
            }),
        ]);
    }
 ...
 */
export function isEmptyResponse(body) {
    ...
    const hasChoices = 'choices' in obj && Array.isArray(obj.choices) &&
        obj.choices.some(c => c !== null && c !== undefined && typeof c === 'object' && Object.keys(c).length > 0);
...
export function attachResponseIdCapture(response, headers, onResponseId) {
