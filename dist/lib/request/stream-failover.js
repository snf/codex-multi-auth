 ...
 */
async function readChunkWithTimeout(readPromise, timeoutMs) {
    let timeoutId;
    try {
 ...
 */
function isStallTimeoutError(error) {
    if (error instanceof StallTimeoutError)
        return true;
    if (!error || typeof error !== "object")
        return false;
    const candidate = error;
    if (candidate.isStallTimeout === true)
        return true;
    return false;
}
/**
 ...
 */
function normalizeRequestInstanceId(value) {
    if (!value)
        return null;
    const trimmed = value.trim();
    if (!trimmed)
        return null;
    if (trimmed.length <= MAX_REQUEST_INSTANCE_ID_LENGTH)
        return trimmed;
    return trimmed.slice(0, MAX_REQUEST_INSTANCE_ID_LENGTH);
}
async function readChunkWithSoftHardTimeout(reader, softTimeoutMs, hardTimeoutMs) {
 ...
 */
export function withStreamingFailover(initialResponse, getFallbackResponse, options = {}) {
    ...
    const softTimeoutMs = Math.max(1_000, Math.floor(options.softTimeoutMs ?? Math.min(defaultHardTimeoutMs, DEFAULT_SOFT_TIMEOUT_MS)));
    const hardTimeoutMs = Math.max(softTimeoutMs, Math.floor(options.hardTimeoutMs ?? defaultHardTimeoutMs));
        ...
        start(controller) {
            ...
            const pump = async () => {
                while (!closed && currentReader) {
                    try {
                        const result = await readChunkWithSoftHardTimeout(currentReader, softTimeoutMs, hardTimeoutMs);
                        if (result.done) {
                            closed = true;
                            controller.close();
                            await releaseCurrentReader();
