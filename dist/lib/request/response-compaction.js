...
function isInputItemArray(value) {
    return Array.isArray(value) && value.every((item) => isRecord(item));
}
function extractCompactedInput(payload) {
    if (!isRecord(payload))
        return undefined;
    if (isInputItemArray(payload.output))
        return payload.output;
    if (isInputItemArray(payload.input))
        return payload.input;
    const response = payload.response;
    if (!isRecord(response))
        return undefined;
    if (isInputItemArray(response.output))
        return response.output;
    if (isInputItemArray(response.input))
        return response.input;
    return undefined;
}
function buildCompactionUrl(requestUrl) {
...
function createFallbackBody(body, trim) {
    if (!Array.isArray(body.input))
        return undefined;
    const trimmedInput = trimInputForFastSession(body.input, trim.maxItems, {
        preferLatestUserOnly: trim.preferLatestUserOnly,
    }) ?? body.input;
    return trimmedInput === body.input ? undefined : { ...body, input: trimmedInput };
}
function createTimedAbortSignal(signal, timeoutMs) {
...
export async function applyResponseCompaction(params) {
    ...
    const { signal, cleanup } = createTimedAbortSignal(params.signal, Math.max(250, params.timeoutMs ?? 4_000));
    try {
...
//# sourceMappingURL=response-compaction.js.map