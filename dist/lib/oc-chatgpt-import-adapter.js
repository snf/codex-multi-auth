...
import { findMatchingAccountIndex, getAccountIdentityKey, normalizeEmailKey, } from "./storage.js";
/**
 ...
 */
function pickNewest(current, candidate) {
...
function sanitizeAccount(account) {
    ...
    const accountId = typeof account.accountId === "string"
        ...
        : undefined;
    const email = typeof account.email === "string" ? account.email.trim() : undefined;
    const accountLabel = typeof account.accountLabel === "string"
    ...
    return {
        ...
        refreshToken,
        addedAt: Number.isFinite(account.addedAt) ? account.addedAt : 0,
 ...
 */
function buildPreviewPayload(storage) {
    return {
        version: 3,
 ...
 */
function findNormalizedAccountIndex(accounts, target) {
    if (!target)
        return null;
 ...
 */
function remapActiveIndex(originalAccounts, originalIndex, normalizedAccounts) {
 ...
 */
function normalizeActiveIndexByFamily(activeIndexByFamily, originalAccounts, normalizedAccounts) {
    if (!activeIndexByFamily)
        return undefined;
    const normalized = {};
    for (const family of MODEL_FAMILIES) {
        const raw = activeIndexByFamily[family];
 ...
 */
function normalizeAccountsForTarget(accounts) {
    const skipped = [];
    const valid = [];
    ...
    const byAccountIdentity = new Map();
 ...
 */
function normalizeStorageForTarget(storage) {
    const { accounts, skipped } = normalizeAccountsForTarget(storage?.accounts ?? []);
    const activeIndex = remapActiveIndex(storage?.accounts, storage?.activeIndex, accounts);
    const activeIndexByFamily = normalizeActiveIndexByFamily(storage?.activeIndexByFamily, storage?.accounts, accounts);
    return {
        storage: {
            version: 3,
 ...
 */
function matchDestination(source, destination, usedIndexes) {
    ...
    if (sourceAccountId && sourceEmail) {
        ...
        const idx = destination.findIndex((account, i) => !usedIndexes.has(i) &&
    ...
    if (sourceAccountId) {
        ...
        for (let i = 0; i < destination.length; i += 1) {
            ...
            if (!account)
                continue;
            if (account.accountId?.trim() !== sourceAccountId)
                continue;
            if (idx !== null) {
                idx = null;
                break;
    ...
    if (sourceEmail) {
        ...
        if (idx >= 0)
            return { index: idx, matchedBy: "email" };
    }
    const idx = destination.findIndex((account, i) => {
        if (usedIndexes.has(i))
            return false;
        if (account.accountId)
            return false;
        if (normalizeEmailKey(account.email))
            return false;
        return account.refreshToken === source.refreshToken;
    });
    if (idx >= 0)
        return { index: idx, matchedBy: "refreshToken" };
    return null;
}
/**
 ...
 */
export function buildOcChatgptImportPayload(source) {
...
export function previewOcChatgptImportMerge(options) {
    ...
    const destinationAccounts = merged.accounts.slice(0, destinationNormalized.storage.accounts.length);
    const usedDestinationIndexes = new Set();
    const toAdd = [];
    const toUpdate = [];
    const toSkip = [
    ...
    for (const account of sourceNormalized.storage.accounts) {
        const match = matchDestination(account, destinationAccounts, usedDestinationIndexes);
        if (!match) {
    ...
    const unchangedDestinationOnly = [];
    for (let i = 0; i < destinationLength; i += 1) {
        if (usedDestinationIndexes.has(i))
            continue;
...
//# sourceMappingURL=oc-chatgpt-import-adapter.js.map