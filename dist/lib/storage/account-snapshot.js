 ...
 */
export async function statSnapshot(path, deps) {
    try {
 ...
 */
export async function describeAccountSnapshot(path, kind, deps) {
    const stats = await deps.statSnapshot(path);
    if (!stats.exists) {
        return { kind, path, index: deps.index, exists: false, valid: false };
    }
    try {
        const { normalized, schemaErrors, storedVersion } = await deps.loadAccountsFromPath(path);
        let resolvedStats = stats;
        for (let attempt = 0; attempt < 2 &&
            (resolvedStats.bytes === undefined || resolvedStats.mtimeMs === undefined); attempt += 1) {
...
//# sourceMappingURL=account-snapshot.js.map