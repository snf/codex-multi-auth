export function normalizeFlaggedStorage(data, deps) {
    if (!deps.isRecord(data) ||
        data.version !== 1 ||
    ...
    for (const rawAccount of data.accounts) {
        if (!deps.isRecord(rawAccount))
            continue;
        const refreshToken = typeof rawAccount.refreshToken === "string"
            ? rawAccount.refreshToken.trim()
            : "";
        if (!refreshToken)
            continue;
        const flaggedAt = typeof rawAccount.flaggedAt === "number"
            ? rawAccount.flaggedAt
            : deps.now();
        const isAccountIdSource = (value) => value === "token" ||
            value === "id_token" ||
            value === "org" ||
            value === "manual";
        const isSwitchReason = (value) => value === "rate-limit" ||
            value === "initial" ||
            value === "rotation" ||
            value === "best" ||
            value === "restore";
        const isCooldownReason = (value) => value === "auth-failure" ||
            value === "network-error" ||
            value === "rate-limit";
        ...
        const normalized = {
            ...
            lastUsed: typeof rawAccount.lastUsed === "number"
                ? rawAccount.lastUsed
                : flaggedAt,
            accountId: typeof rawAccount.accountId === "string"
                ? rawAccount.accountId
                : undefined,
            accountIdSource,
            accountLabel: typeof rawAccount.accountLabel === "string"
                ? rawAccount.accountLabel
                : undefined,
            email: typeof rawAccount.email === "string" ? rawAccount.email : undefined,
            enabled: typeof rawAccount.enabled === "boolean"
                ? rawAccount.enabled
                : undefined,
            lastSwitchReason,
            rateLimitResetTimes,
            coolingDownUntil: typeof rawAccount.coolingDownUntil === "number"
                ? rawAccount.coolingDownUntil
                : undefined,
            cooldownReason,
            flaggedAt,
            flaggedReason: typeof rawAccount.flaggedReason === "string"
                ? rawAccount.flaggedReason
                : undefined,
            lastError: typeof rawAccount.lastError === "string"
...
//# sourceMappingURL=flagged-storage.js.map