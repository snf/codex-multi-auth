 ...
 */
export function toStorageError(message, error, path) {
    return new StorageError(message, extractErrorCode(error), path, formatStorageErrorHint(error, path), error instanceof Error ? error : undefined);
}
