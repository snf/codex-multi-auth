...
export function isNonInteractiveMode() {
    if (process.env.FORCE_INTERACTIVE_MODE === "1")
        return false;
    if (!input.isTTY || !output.isTTY)
        return true;
    if (process.env.CODEX_TUI === "1")
        return true;
    if (process.env.CODEX_DESKTOP === "1")
        return true;
...
function resolveAccountSourceIndex(account) {
    const sourceIndex = typeof account.sourceIndex === "number" && Number.isFinite(account.sourceIndex)
...
function warnUnresolvableAccountSelection(account) {
...
async function promptLoginModeFallback(existingAccounts) {
    ...
    try {
        ...
        while (true) {
            ...
            if (normalized === "x" || normalized === "fix")
                return { mode: "fix" };
...
export async function promptLoginMode(existingAccounts, options = {}) {
    if (isNonInteractiveMode()) {
        return { mode: "add" };
    }
    if (!isTTY()) {
        ...
        switch (action.type) {
            ...
            case "select-account": {
                ...
                if (accountAction === "delete") {
                    ...
                    if (index >= 0)
                        return { mode: "manage", deleteAccountIndex: index };
                ...
                if (accountAction === "set-current") {
                    const index = resolveAccountSourceIndex(action.account);
                    if (index >= 0)
                        return { mode: "manage", switchAccountIndex: index };
                    warnUnresolvableAccountSelection(action.account);
                    continue;
                }
                if (accountAction === "refresh") {
                    const index = resolveAccountSourceIndex(action.account);
                    if (index >= 0)
                        return { mode: "manage", refreshAccountIndex: index };
                    warnUnresolvableAccountSelection(action.account);
                    continue;
                }
                if (accountAction === "toggle") {
                    const index = resolveAccountSourceIndex(action.account);
                    if (index >= 0)
                        return { mode: "manage", toggleAccountIndex: index };
                    warnUnresolvableAccountSelection(action.account);
                    continue;
                }
                continue;
...
export async function promptAccountSelection(candidates, options = {}) {
    if (candidates.length === 0)
        return null;
    const defaultIndex = typeof options.defaultIndex === "number" && Number.isFinite(options.defaultIndex)
