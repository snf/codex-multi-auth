...
const LEGACY_CODEX_HOME_AUTH_CONFIG_PATH = join(CODEX_HOME_DIR, "openai-codex-auth-config.json");
const LEGACY_CODEX_CONFIG_PATH = join(LEGACY_CODEX_DIR, "codex-multi-auth-config.json");
const LEGACY_CODEX_AUTH_CONFIG_PATH = join(LEGACY_CODEX_DIR, "openai-codex-auth-config.json");
...
function resolvePluginConfigPath() {
    ...
    if (IS_CUSTOM_CODEX_HOME && existsSync(LEGACY_CODEX_HOME_CONFIG_PATH)) {
        logConfigWarnOnce(`Using legacy config path ${LEGACY_CODEX_HOME_CONFIG_PATH}. ` +
    ...
    if (IS_CUSTOM_CODEX_HOME && existsSync(LEGACY_CODEX_HOME_AUTH_CONFIG_PATH)) {
        logConfigWarnOnce(`Using legacy config path ${LEGACY_CODEX_HOME_AUTH_CONFIG_PATH}. ` +
            `Please migrate to ${CONFIG_PATH}.`);
        return LEGACY_CODEX_HOME_AUTH_CONFIG_PATH;
    }
    if (existsSync(LEGACY_CODEX_AUTH_CONFIG_PATH)) {
        logConfigWarnOnce(`Using legacy config path ${LEGACY_CODEX_AUTH_CONFIG_PATH}. ` +
            `Please migrate to ${CONFIG_PATH}.`);
 ...
 */
export function getResponseContinuation(pluginConfig) {
    return resolveBooleanSetting("CODEX_AUTH_RESPONSE_CONTINUATION", pluginConfig.responseContinuation, false);
}
/**
 ...
 */
export function getBackgroundResponses(pluginConfig) {
    return resolveBooleanSetting("CODEX_AUTH_BACKGROUND_RESPONSES", pluginConfig.backgroundResponses, false);
}
/**
 ...
 */
function withExplainEnvUnset(envNames, run) {
...
function configExplainValuesEqual(left, right) {
    return JSON.stringify(left) === JSON.stringify(right);
}
function resolveConfigExplainSource(entry, pluginConfig, storedRecord, storageKind) {
    ...
    const hasStoredSource = (storageKind === "unified" || storageKind === "file") &&
        storedRecord !== null &&
...
function normalizeConfigExplainValue(value) {
    if (typeof value === "number" && !Number.isFinite(value)) {
...
export function getPluginConfigExplainReport() {
