...
import { evaluateForecastAccounts, isHardRefreshFailure, recommendForecastAccount, } from "../forecast.js";
import { extractAccountEmail, extractAccountId, formatAccountLabel, sanitizeEmail, } from "../accounts.js";
import { loadQuotaCache, saveQuotaCache } from "../quota-cache.js";
import { fetchCodexQuotaSnapshot } from "../quota-probe.js";
import { queuedRefresh } from "../refresh-queue.js";
import { findMatchingAccountIndex, getStoragePath, loadAccounts, loadFlaggedAccounts, setStoragePath, withAccountStorageTransaction, withAccountAndFlaggedStorageTransaction, withFlaggedStorageTransaction, } from "../storage.js";
import { getCodexCliAuthPath, getCodexCliConfigPath, loadCodexCliState, } from "../codex-cli/state.js";
...
export function parseVerifyFlaggedArgs(args) {
    const options = {
        dryRun: false,
        json: false,
        restore: true,
...
function hasAccountStorageMutation(before, after) {
    return (before.refreshToken !== after.refreshToken
...
function collectAccountStorageMutations(beforeAccounts, afterAccounts) {
    const mutations = [];
    for (let i = 0; i < afterAccounts.length; i += 1) {
        ...
        if (!before || !after)
            continue;
        if (!hasAccountStorageMutation(before, after))
            continue;
        mutations.push({
...
function applyAccountStorageMutations(storage, mutations) {
    for (const mutation of mutations) {
        const targetIndex = findMatchingAccountIndex(storage.accounts, mutation.before, {
...
function findMatchingFlaggedAccountIndex(accounts, target) {
...
function findFlaggedAccountIndexByStableIdentity(accounts, target) {
    const targetEmail = sanitizeEmail(target.email);
    return accounts.findIndex((account) => {
        if (target.accountId && account.accountId === target.accountId) {
            if (!targetEmail) {
...
function hasFlaggedRefreshTokenDrift(accounts, target) {
...
function applyFlaggedStorageMutations(flaggedStorage, mutations) {
    for (const mutation of mutations) {
        const targetIndex = findMatchingFlaggedAccountIndex(flaggedStorage.accounts, mutation.before);
        if (targetIndex < 0) {
            continue;
        }
        if (mutation.after) {
...
function upsertRecoveredFlaggedAccount(storage, flagged, refreshResult, now, deps) {
    ...
    const { accountId: nextAccountId, accountIdSource: nextAccountIdSource } = deps.resolveStoredAccountIdentity(flagged.accountId, flagged.accountIdSource, tokenAccountId);
...
function maskDoctorEmail(value) {
    if (!value)
        return undefined;
...
function redactDoctorIdentifier(value) {
    if (!value)
        return undefined;
...
function getDoctorRefreshTokenKey(refreshToken) {
    if (typeof refreshToken !== "string")
        return undefined;
    const trimmed = refreshToken.trim();
    return trimmed || undefined;
}
function applyDoctorFixes(storage, deps) {
    let changed = false;
    const actions = [];
    const recordActiveIndexAction = () => {
        if (actions.some((action) => action.key === "active-index"))
            return;
        actions.push({
            key: "active-index",
...
export async function runVerifyFlagged(args, deps) {
    ...
    const flaggedMutations = [];
    const now = Date.now();
    const collectRefreshChecks = async (accounts) => {
        const refreshChecks = [];
    ...
    const applyRefreshChecks = (storage, refreshChecks) => {
            ...
            if (result.type === "success") {
                if (!options.restore) {
                    ...
                    const nextFlagged = {
                        ...
                        email: sanitizeEmail(extractAccountEmail(result.access, result.idToken))
                            ?? flagged.email,
                        lastUsed: now,
                        lastError: undefined,
                    };
                ...
                const upsertResult = upsertRecoveredFlaggedAccount(storage, flagged, result, now, deps);
    ...
    if (options.restore) {
        if (options.dryRun) {
            applyRefreshChecks((await loadAccounts()) ?? createEmptyAccountStorage(), refreshChecks);
        }
        else {
            await withAccountAndFlaggedStorageTransaction(async (loadedStorage, persist, loadedFlaggedStorage) => {
                ...
                const staleRefreshChecks = refreshChecks.filter((check) => hasFlaggedRefreshTokenDrift(nextFlaggedStorage.accounts, check.flagged));
                const safeRefreshChecks = refreshChecks.filter((check) => !hasFlaggedRefreshTokenDrift(nextFlaggedStorage.accounts, check.flagged));
                ...
                for (const check of staleRefreshChecks) {
                    reports.push({
                        ...
                        label: check.label,
                        outcome: "restore-skipped",
                        message: "Skipped restore because flagged refresh token changed before persistence",
                    });
                }
    ...
    if (!options.dryRun && !options.restore && flaggedChanged) {
        await withFlaggedStorageTransaction(async (loadedFlaggedStorage, persist) => {
            const nextFlaggedStorage = structuredClone(loadedFlaggedStorage);
            const staleFlaggedMutations = flaggedMutations.filter((mutation) => hasFlaggedRefreshTokenDrift(nextFlaggedStorage.accounts, mutation.before));
            const safeFlaggedMutations = flaggedMutations.filter((mutation) => !hasFlaggedRefreshTokenDrift(nextFlaggedStorage.accounts, mutation.before));
            for (const mutation of staleFlaggedMutations) {
                const staleReport = reports.find((report) => report.index === mutation.index && report.label === mutation.label);
                if (staleReport) {
                ...
                reports.push({
                    ...
                    outcome: "restore-skipped",
                    message: "Skipped flagged update because refresh token changed before persistence",
                });
            }
    ...
    console.log(deps.stylePromptText(`Checking ${flaggedStorage.accounts.length} flagged account(s)...`, "accent"));
    for (const report of reports) {
        ...
        const marker = report.outcome === "restored"
            ...
            : report.outcome === "healthy-flagged" || report.outcome === "restore-skipped"
                ? "!"
                : "✗";
        console.log(`${deps.stylePromptText(marker, tone)} ${deps.stylePromptText(`${report.index + 1}. ${report.label}`, "accent")} ${deps.stylePromptText("|", "muted")} ${deps.styleAccountDetailText(report.message, tone)}`);
    }
    console.log("");
...
export async function runFix(args, deps) {
    if (args.includes("--help") || args.includes("-h")) {
        printFixUsage();
        return 0;
    }
    ...
    const accountMutations = collectAccountStorageMutations(originalAccounts, storage.accounts);
    ...
    console.log(deps.stylePromptText(`Auto-fix scan (${options.dryRun ? "preview" : "apply"})`, "accent"));
    console.log(deps.formatResultSummary([
            ...
            const tone = report.outcome === "healthy"
                ...
                : report.outcome === "disabled-hard-failure"
                    ...
                    : report.outcome === "warning-soft-failure"
                        ? "warning"
                        : "muted";
            console.log(`${deps.stylePromptText(prefix, tone)} ${deps.stylePromptText(`${report.index + 1}. ${report.label}`, "accent")} ${deps.stylePromptText("|", "muted")} ${deps.styleAccountDetailText(report.message, tone === "success" ? "muted" : tone)}`);
        }
    }
    else {
        console.log("");
    ...
    if (display.showRecommendations) {
        ...
        if (recommendation.recommendedIndex !== null) {
            ...
            console.log(`${deps.stylePromptText("Best next account:", "accent")} ${deps.stylePromptText(String(target), "success")}`);
            ...
            if (recommendation.recommendedIndex !== activeIndex) {
                console.log(`${deps.stylePromptText("Switch now with:", "accent")} codex auth switch ${target}`);
            }
        }
        else {
...
export async function runDoctor(args, deps) {
    ...
    const storageForChecks = options.fix && storage ? structuredClone(storage) : storage;
    let fixChanged = false;
    let storageFixChanged = false;
    let structuralFixActions = [];
    const supplementalFixActions = [];
    let doctorRefreshMutation = null;
    let pendingCodexActiveSync = null;
    ...
    if (!storageForChecks || storageForChecks.accounts.length === 0) {
        ...
        const activeExists = activeIndex >= 0 && activeIndex < storageForChecks.accounts.length;
        addCheck({
            key: "active-index",
        ...
        const disabledCount = storageForChecks.accounts.filter((a) => a.enabled === false).length;
        addCheck({
            key: "enabled-accounts",
            severity: disabledCount >= storageForChecks.accounts.length ? "error" : "ok",
            message: disabledCount >= storageForChecks.accounts.length
        ...
        const now = Date.now();
        const forecastResults = evaluateForecastAccounts(storageForChecks.accounts.map((account, index) => ({
            index,
            account,
            isCurrent: index === activeIndex,
            now,
        ...
        if (activeExists) {
            ...
            addCheck({
                ...
                details: `manager=${formatDoctorIdentitySummary({
            ...
            if (options.fix && activeAccount) {
                ...
                if (!canSyncActiveAccount) {
                    if (options.dryRun) {
                        ...
                        if (refreshResult.type === "success") {
                            const refreshedEmail = sanitizeEmail(extractAccountEmail(refreshResult.access, refreshResult.idToken));
    ...
    if (options.fix
        ...
        await withAccountStorageTransaction(async (loadedStorage, persist) => {
            ...
            if (doctorRefreshMutation) {
                ...
                const fallbackTargetIndex = fallbackActiveIndex >= 0 && fallbackActiveIndex < nextStorage.accounts.length
                    ? fallbackActiveIndex
                    : undefined;
                const targetIndex = findMatchingAccountIndex(nextStorage.accounts, doctorRefreshMutation.match, {
                    allowUniqueAccountIdFallbackWithoutEmail: true,
                })
