...
export async function promptExperimentalSettingsMenu(params) {
    ...
    while (true) {
        ...
        ], params.getExperimentalSelectOptions(params.ui, copy.experimentalHelpMenu, params.mapExperimentalMenuHotkey));
        ...
        if (actionType === "decrease-refresh-interval") {
            draft = {
                ...draft,
                proactiveRefreshIntervalMs: Math.max(60_000, (draft.proactiveRefreshIntervalMs ?? 60000) - 60000),
            };
            continue;
        }
        if (actionType === "increase-refresh-interval") {
            draft = {
                ...draft,
                proactiveRefreshIntervalMs: Math.min(600000, (draft.proactiveRefreshIntervalMs ?? 60000) + 60000),
            };
            continue;
        }
        if (actionType === "backup") {
            ...
            try {
                const backupName = (await prompt.question(copy.experimentalBackupPrompt)).trim();
                ...
                try {
                    ...
                    ], params.getExperimentalSelectOptions(params.ui, copy.experimentalHelpStatus, params.mapExperimentalStatusHotkey));
                }
                catch (error) {
        ...
        const plan = await params.planOcChatgptSync({
            source,
            destination: targetKind === "target"
                ? params.getTargetDestination(targetState)
                : null,
            dependencies: targetKind === "target"
        ...
        const review = await params.select([
            {
            ...
            { label: copy.backNoSave, value: { type: "back" }, color: "red" },
        ], params.getExperimentalSelectOptions(params.ui, copy.experimentalHelpPreview, (raw) => {
...
//# sourceMappingURL=experimental-settings-prompt.js.map