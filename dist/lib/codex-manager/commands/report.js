...
import { formatQuotaSnapshotLine, } from "../../quota-probe.js";
...
function printReportUsage(logInfo) {
    logInfo([
...
export async function runReportCommand(args, deps) {
    ...
    const coolingCount = storage
        ? storage.accounts.filter((account) => typeof account.coolingDownUntil === "number" &&
            account.coolingDownUntil > now).length
        : 0;
    ...
    if (options.outPath) {
        ...
        await (deps.writeFile ?? defaultWriteFile)(outputPath, `${JSON.stringify(report, null, 2)}\n`);
    }
    if (options.json) {
    ...
    logInfo(`Accounts: ${report.accounts.total} total (${report.accounts.enabled} enabled, ${report.accounts.disabled} disabled, ${report.accounts.coolingDown} cooling, ${report.accounts.rateLimited} rate-limited)`);
    if (report.activeIndex !== null) {
        logInfo(`Active account: ${report.activeIndex}`);
    }
    logInfo(`Forecast: ${report.forecast.summary.ready} ready, ${report.forecast.summary.delayed} delayed, ${report.forecast.summary.unavailable} unavailable`);
    if (report.forecast.recommendation.recommendedIndex !== null) {
        logInfo(`Recommendation: account ${report.forecast.recommendation.recommendedIndex + 1} (${report.forecast.recommendation.reason})`);
    }
    else {
    ...
    if (options.explain) {
        ...
        for (const account of report.forecast.accounts) {
            logInfo(`Account ${account.index + 1}: ${account.availability}, ${account.riskLevel} risk (${account.riskScore})`);
