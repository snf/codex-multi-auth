import { buildForecastExplanation, } from "../../forecast.js";
import { resolveNormalizedModel } from "../../request/helpers/model-map.js";
function joinStyledSegments(parts, styleText) {
    if (parts.length === 0)
        return "";
    return parts.join(styleText(" | ", "muted"));
}
function printForecastUsage(logInfo) {
    logInfo([
        "Usage:",
...
function serializeForecastResults(results, liveQuotaByIndex, refreshFailures, formatQuotaSnapshotLine) {
...
export async function runForecastCommand(args, deps) {
    ...
    for (let i = 0; i < storage.accounts.length; i += 1) {
        ...
        let probeAccountId = account.accountId ?? deps.extractAccountId(account.accessToken);
        ...
        if (!probeAccessToken || !probeAccountId) {
            probeErrors.push(`${deps.formatAccountLabel(account, i)}: missing accountId for live probe`);
            continue;
        }
        try {
            ...
            if (workingQuotaCache) {
                ...
                if (nextAccount) {
                    quotaCacheChanged =
                        deps.updateQuotaCacheForAccount(workingQuotaCache, nextAccount, liveQuota, storage.accounts, quotaEmailFallbackState ?? undefined) || quotaCacheChanged;
                }
            }
        }
        catch (error) {
    ...
    const explanation = buildForecastExplanation(forecastResults, recommendation);
    if (options.json) {
        ...
        logInfo(JSON.stringify({
            ...
            probeErrors,
            accounts: serializeForecastResults(forecastResults, liveQuotaByIndex, refreshFailures, deps.formatQuotaSnapshotLine),
        }, null, 2));
        return 0;
    }
    logInfo(deps.stylePromptText(`Best-account preview (${storage.accounts.length} account(s), model ${requestedModel}, live check ${options.live ? "on" : "off"})`, "accent"));
    ...
    for (const result of forecastResults) {
        ...
        const waitLabel = result.waitMs > 0
            ? deps.stylePromptText(`wait ${deps.formatWaitTime(result.waitMs)}`, "muted")
            : "";
        ...
        const riskLabel = deps.stylePromptText(`${result.riskLevel} risk (${result.riskScore})`, deps.riskTone(result.riskLevel));
        const availabilityLabel = deps.stylePromptText(result.availability, deps.availabilityTone(result.availability));
        const rowParts = [availabilityLabel, riskLabel];
        if (waitLabel)
            rowParts.push(waitLabel);
        logInfo(`${indexLabel} ${accountLabel} ${deps.stylePromptText("|", "muted")} ${joinStyledSegments(rowParts, deps.stylePromptText)}`);
        if (display.showForecastReasons && result.reasons.length > 0) {
            logInfo(`   ${deps.stylePromptText(result.reasons.slice(0, 3).join("; "), "muted")}`);
        }
        const liveQuota = liveQuotaByIndex.get(result.index);
        if (display.showQuotaDetails && liveQuota) {
            logInfo(`   ${deps.stylePromptText("quota:", "accent")} ${deps.styleQuotaSummary(deps.formatCompactQuotaSnapshot(liveQuota))}`);
        }
    }
    if (!display.showPerAccountRows) {
    ...
    if (display.showRecommendations) {
        if (recommendation.recommendedIndex !== null) {
            ...
            if (account) {
                logInfo(`${deps.stylePromptText("Best next account:", "accent")} ${deps.stylePromptText(`${index + 1} (${account.label})`, "success")}`);
                logInfo(`${deps.stylePromptText("Why:", "accent")} ${deps.stylePromptText(recommendation.reason, "muted")}`);
                if (index !== activeIndex) {
                    logInfo(`${deps.stylePromptText("Switch now with:", "accent")} codex auth switch ${index + 1}`);
                }
            }
        }
        else {
    ...
    if (options.explain) {
        logInfo(`${deps.stylePromptText("Explain:", "accent")} ${deps.stylePromptText(explanation.recommendationReason, "muted")}`);
        for (const item of explanation.considered) {
            const selectedLabel = item.selected ? " selected" : "";
            const waitLabel = item.waitMs > 0 ? `, wait ${deps.formatWaitTime(item.waitMs)}` : "";
            logInfo(`  ${deps.stylePromptText(item.selected ? "*" : "-", item.selected ? "success" : "muted")} ${deps.stylePromptText(`${item.index + 1}. ${item.label}${item.isCurrent ? " [current]" : ""}: ${item.availability}, ${item.riskLevel} risk (${item.riskScore})${waitLabel}${selectedLabel}`, item.selected ? "success" : "muted")}`);
            if (item.reasons.length > 0) {
                logInfo(`    ${deps.stylePromptText(item.reasons.slice(0, 3).join("; "), "muted")}`);
            }
        }
    }
    if (display.showLiveProbeNotes && probeErrors.length > 0) {
        logInfo("");
        logInfo(deps.stylePromptText(`Live check notes (${probeErrors.length}):`, "warning"));
        for (const error of probeErrors) {
            logInfo(`  ${deps.stylePromptText("-", "warning")} ${deps.stylePromptText(error, "muted")}`);
        }
    }
    if (workingQuotaCache && quotaCacheChanged) {
        await deps.saveQuotaCache(workingQuotaCache);
