import { classifyAccessTokenFailureForReauth, clearAccountReauthRequired, markAccountReauthRequired, } from "../../account-reauth.js";
import { resolveNormalizedModel } from "../../request/helpers/model-map.js";
export async function runBestCommand(args, deps) {
    ...
    const probeErrors = [];
    let changed = false;
    const persistProbeChangesIfNeeded = async (beforeSave) => {
        if (!changed)
            return;
        beforeSave?.();
    ...
    for (let i = 0; i < storage.accounts.length; i += 1) {
        ...
        if (!account || !options.live || account.enabled === false)
            continue;
    ...
    if (currentIndex === bestIndex) {
        ...
        if (!alreadyBestSynced && !options.json) {
            logWarn("Codex auth sync did not complete. Multi-auth routing will still use this account.");
        }
        if (options.json) {
            logInfo(JSON.stringify({
            ...
            logInfo(`Already on best account ${bestIndex + 1}: ${deps.formatAccountLabel(bestAccount, bestIndex)}`);
    ...
    if (options.json) {
        ...
        logInfo(`Switched to best account ${parsed}: ${deps.formatAccountLabel(bestAccount, bestIndex)}${wasDisabled ? " (re-enabled)" : ""}`);
        logInfo(`Reason: ${recommendation.reason}`);
        printProbeNotes();
        if (!synced) {
            logWarn("Codex auth sync did not complete. Multi-auth routing will still use this account.");
