...
import { evaluateForecastAccounts, recommendForecastAccount, } from "../forecast.js";
import { loadQuotaCache } from "../quota-cache.js";
import { fetchCodexQuotaSnapshot } from "../quota-probe.js";
import { queuedRefresh } from "../refresh-queue.js";
import { getNamedBackups, formatStorageErrorHint, getStoragePath, loadAccounts, loadFlaggedAccounts, restoreAccountsFromBackup, saveAccounts, setStoragePath, StorageError, } from "../storage.js";
import { setCodexCliActiveSelection } from "../codex-cli/writer.js";
import { MODEL_FAMILIES } from "../prompts/codex.js";
import { RefreshLeaseCoordinator } from "../refresh-lease.js";
import { UI_COPY } from "../ui/copy.js";
import { confirm } from "../ui/confirm.js";
import { applyUiThemeFromDashboardSettings, configureUnifiedSettings, } from "./settings-hub.js";
import { parseAuthLoginArgs, parseBestArgs, printBestUsage, printUsage, } from "./help.js";
const liveBestLeaseCoordinator = RefreshLeaseCoordinator.fromEnvironment();
const serializedLiveBestByStorage = new Map();
async function withSerializedBestLiveRun(storagePath, action) {
    const queueEntry = serializedLiveBestByStorage.get(storagePath) ?? { tail: Promise.resolve() };
    ...
    try {
        ...
        try {
            ...
            releaseQueue();
            if (serializedLiveBestByStorage.get(storagePath) === queueEntry &&
                queueEntry.tail === currentTail) {
...
function clampPreservedActiveIndexByFamily(storage, targetIndex) {
    ...
    for (const family of MODEL_FAMILIES) {
        ...
        if (typeof candidate !== "number" || !Number.isInteger(candidate))
            continue;
        nextByFamily[family] = Math.min(maxIndex, Math.max(0, candidate));
    }
    nextByFamily.codex = targetIndex;
    storage.activeIndexByFamily = nextByFamily;
}
export async function persistAndSyncSelectedAccount({ storage, targetIndex, parsed, switchReason, initialSyncIdToken, preserveActiveIndexByFamily = false, helpers, }) {
    ...
    if (!helpers.hasUsableAccessToken(account, switchNow)) {
        ...
        if (refreshResult.type === "success") {
            ...
            console.warn(`Switch validation refresh failed for account ${parsed}: ${helpers.normalizeFailureDetail(refreshResult.message, refreshResult.reason)}.`);
        }
    }
...
export async function runSwitch(args, helpers) {
    setStoragePath(null);
    ...
    if (!synced) {
        console.warn(`Switched account ${parsed} locally, but Codex auth sync did not complete. Multi-auth routing will still use this account.`);
    }
    console.log(`Switched to account ${parsed}: ${formatAccountLabel(account, targetIndex)}${wasDisabled ? " (re-enabled)" : ""}`);
    return 0;
}
export async function runBest(args, helpers) {
    ...
    const execute = async () => {
        ...
        const probeIdTokenByIndex = new Map();
        const probeRefreshedIndices = new Set();
        const probeErrors = [];
        let changed = false;
        const printProbeNotes = () => {
        ...
        for (let i = 0; i < storage.accounts.length; i += 1) {
            ...
            try {
                ...
                const message = helpers.normalizeFailureDetail(error instanceof Error ? error.message : String(error), undefined);
        ...
        if (currentIndex === bestIndex) {
            const shouldSyncCurrentBest = probeRefreshedIndices.has(bestIndex) || probeIdTokenByIndex.has(bestIndex);
            let alreadyBestSynced;
            if (changed) {
...
export async function runAuthLogin(args, deps) {
    ...
    loginFlow: while (true) {
        ...
        if (existingStorage && existingStorage.accounts.length > 0) {
            while (true) {
                ...
                const quotaTtlMs = displaySettings.menuQuotaTtlMs ?? deps.defaultMenuQuotaRefreshTtlMs;
                if (shouldAutoFetchLimits && !pendingMenuQuotaRefresh) {
                    const staleCount = deps.countMenuQuotaRefreshTargets(currentStorage, quotaCache, quotaTtlMs);
                    if (staleCount > 0) {
                        if (showFetchStatus) {
                            menuQuotaRefreshStatus = `${UI_COPY.mainMenu.loadingLimits} [0/${staleCount}]`;
                        }
                        pendingMenuQuotaRefresh = deps.refreshQuotaCacheForMenu(currentStorage, quotaCache, quotaTtlMs, (current, total) => {
                            if (!showFetchStatus)
                                return;
                ...
                const menuResult = await promptLoginMode(deps.toExistingAccountInfo(currentStorage, quotaCache, displaySettings), {
        ...
        while (true) {
            ...
            const signInMode = preferManualMode
                ? "manual"
                : await deps.promptOAuthSignInMode(latestNamedBackup, onboardingBackupDiscoveryWarning);
            ...
            if (signInMode === "restore-backup") {
                ...
                applyUiThemeFromDashboardSettings(displaySettings);
                try {
                    await deps.runActionPanel("Load Backup", `Loading ${selectedBackup.fileName}`, async () => {
