...
function statusBadge(status) {
    ...
    const withTone = (label, tone) => {
        if (ui.v2Enabled)
            return formatUiBadge(ui, label, tone);
        if (tone === "accent")
...
function accountTitle(account) {
    ...
    const base = sanitizeTerminalText(account.email) ||
...
function accountRowColor(account) {
...
function parseLeftPercentFromSummary(summary, windowLabel) {
    const segments = summary.split("|");
    for (const segment of segments) {
        const trimmed = segment.trim().toLowerCase();
        if (!trimmed.startsWith(`${windowLabel} `))
            continue;
...
function formatLimitCooldown(resetAtMs) {
...
function formatQuotaBar(leftPercent, ui) {
    const width = 10;
    ...
    if (leftPercent === null)
        return `${ANSI.dim}${emptyText}${ANSI.reset}`;
...
function formatQuotaPercent(leftPercent, ui) {
    if (leftPercent === null)
        return null;
...
function formatQuotaWindow(label, leftPercent, resetAtMs, showCooldown, ui) {
...
function formatQuotaSummary(account, ui) {
...
function formatAccountHint(account, ui) {
    const withKey = (key, value, tone) => {
        if (!ui.v2Enabled)
            return `${key} ${value}`;
...
async function promptSearchQuery(current) {
...
function authMenuFocusKey(action) {
    switch (action.type) {
        case "select-account":
...
export async function showAuthMenu(accounts, options = {}) {
    ...
    while (true) {
        ...
        const duplicateQuickSwitchNumbers = new Set();
        ...
        if (visibleAccounts.length === 0) {
            ...
            items.push(...visibleAccounts.map((account) => {
        ...
        const result = await select(items, {
            ...
            onInput: (input, context) => {
                ...
                if (Number.isFinite(parsed) && parsed >= 1 && parsed <= 9) {
                    ...
                    if (direct) {
                        return { type: "set-current-account", account: direct };
                    }
                }
                ...
                if (selected.value.type !== "select-account")
                    return undefined;
                return undefined;
            },
