...
function normalizeIndexCandidate(value, fallback) {
...
function normalizeStoredFamilyIndexes(storage) {
    ...
    const clamped = count === 0 ? 0 : Math.max(0, Math.min(normalizedActiveIndex, count - 1));
    if (storage.activeIndex !== clamped) {
        storage.activeIndex = clamped;
    }
    storage.activeIndexByFamily = storage.activeIndexByFamily ?? {};
    for (const family of MODEL_FAMILIES) {
        const hasFamilyIndex = Object.prototype.hasOwnProperty.call(storage.activeIndexByFamily, family);
        ...
        const resolved = typeof raw === "number"
            ...
            : storage.activeIndex;
        const familyIndex = count === 0 ? 0 : Math.max(0, Math.min(resolved, count - 1));
 ...
 */
export function syncAccountStorageFromCodexCli(current) {
    ...
    const changed = previousActive !== next.activeIndex ||
